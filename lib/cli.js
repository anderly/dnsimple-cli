...
_.extend(DnsimpleCli.prototype, {
  ...
  recordError: function(err) {
    if (err && err.stack) {
      try {
        fs.writeFileSync('dnsimple.err', (new Date()) + ':\n' +
            util.inspect(err) + '\n' + err.stack + '\n');
        (log.format().json ? log.error : log.info)('Error information has been recorded to dnsimple.err');
      } catch(err2) {
  ...
  */
  ...
  enableAutoComplete: function() {
    ...
    root.autoComplete = omelette('dnsimple');
    ...
    function handleAutocomplete(fragment, word, line) {
      ...
      args.shift(); // discard "dnsimple" word
  ...
  harvestPlugins: function() {
    ...
    if (!plugins) {
      var commands = scan(path.join(basePath, 'commands'), true);
