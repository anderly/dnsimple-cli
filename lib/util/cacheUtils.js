...
function getDomainsFile(subscription) {
  if (!subscription) {
    throw new Error('No current subscription when reading domains file');
  }
  return path.join(utils.dnsimpleDir(), 'domains.' + subscription + '.json');
}
...
function saveDomains(subscription, domains, cb) {
  var cacheFile = getDomainsFile(subscription);
  return fs.writeFile(cacheFile, JSON.stringify(domains), function (err) {
    if (cb) {
      cb(err, domains);
    }
  });
...
function clear() {
  var isDeleted = false;
  if (utils.pathExistsSync(utils.dnsimpleDir())) {
    var cacheFiles = fs.readdirSync(utils.dnsimpleDir());
    for (var i = 0; i < cacheFiles.length; ++i) {
      if (/domains[.].+[.]json/.test(cacheFiles[i]) ||
         /spaces[.].+[.]json/.test(cacheFiles[i]) ||
         /commitids[.].+[.]json/.test(cacheFiles[i])) {
        fs.unlinkSync(path.join(utils.dnsimpleDir(), cacheFiles[i]));
        isDeleted = true;
      }
    }
  }
...
CacheUtils.saveDomains = saveDomains;
