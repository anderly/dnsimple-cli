...
DnsimpleClientFactory.prototype.create = function (callback) {
  ...
  this.credentials.authenticateRequest(function (err, scheme, token) {
    ...
    var apiEndpoint = self.credentials.authConfig.resourceId.replace('https://','');
    var dns = new dnsimple({ email: self.credentials.userId, token: token, hostname: apiEndpoint });
    callback(null, dns);
  });
};
