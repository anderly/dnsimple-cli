...
exports.getBlobName =
    function getBlobName(cli, storageClient, location, affinityGroup, newName, blob, prefix, filename, callback) {

      if (blob) {
        if (blob[0] === '/') {
          utils.getOrCreateBlobStorage(cli, storageClient, location, affinityGroup, newName, function (error, blobEndpoint) {
            if (error) {
              callback(error);
            } else {
