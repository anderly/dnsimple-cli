...
exports.init = function (cli) {
  ...
  function createClient(options) {
    var currentSubscription = profile.current.getSubscription(options.subscription),
        client = azureCommon.requestPipeline.create(
          ...
          log.createLogFilter(),
          UserAgentFilter.create(utils.getUserAgent()),
  ...
  function getMobileChannel(options) {
    ...
    var channel = new PipelineChannel(client, webResource)
        .path('services')
        .path('mobileservices')
  ...
  */
  ...
  mobile.trackAsyncOperation = function (options, requestId, callback) {
    function waitOne() {
      ...
      var asyncChannel = new PipelineChannel(client, webResource)
        .path('operations')
        .path(requestId)
        .header('Accept', 'application/json');
  ...
  mobile.command('create [servicename] [username] [password]')
        ...
        .option('-s, --subscription <id>', $('the subscription id'))
        .option('-b, --backend <backend>', $('backend of mobile service'))
