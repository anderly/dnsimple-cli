...
__.extend(WebsitesClient.prototype, {
  ...
  createWebsiteExtensionsClient: function (context, _) {
    var service = utils._createWebSiteExtensionsClient(
  ...
  getSites: function (context, _) {
    ...
    var progress;
    var service = self.createWebsiteManagementService(context.subscription);
  ...
  getHostNameSuffix: function (subscription, _) {
    ...
    if (!self.hostNameSuffix) {
      ...
      if (subscriptionId) {
        var websiteManagementService = self.createWebsiteManagementService(subscriptionId);
  ...
  getPublishingUser: function (context, _) {
    ...
    if (administrators.length === 0) {
      try {
        ...
        var websiteManagementService = self.createWebsiteManagementService(context.subscription);
