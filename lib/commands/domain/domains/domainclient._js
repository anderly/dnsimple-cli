...
__.extend(DomainClient.prototype, {
  ...
  pushDomain: function (context, _) {
    ...
    try {
      // we're returning .domain here because the npm dnsimple package isn't yet doing that for the push call
  ...
  autoRenewDomain: function (context, _) {
    var self = this;

    var dns = self.createDnsimpleClient(context.subscription).create(_);

    var domainName = context.domain.name;
    var autoRenew = context.flags.autoRenew;

    var result;
    var progress = self.cli.interaction.progress(util.format($((autoRenew ? 'Enabling' : 'Disabling') + ' auto-renewal for domain %s'), domainName));
    try {
      result = dns.domains.autorenew(domainName, autoRenew, _);
    }
    finally {
      progress.end();
    }
  ...
  getMembers: function (context, _) {
    ...
    var members;
    var progress = self.cli.interaction.progress(util.format($('Getting members for domain %s'), domainName));
    try {
      var members = dns.domains.memberships.list(domainName, _);
    }
    finally {
      progress.end();
    }
  ...
  addMember: function (context, _) {
    var self = this;

    var dns = self.createDnsimpleClient(context.subscription).create(_);
    var domainName = context.domain.name;
    var memberEmail = context.member;

    var member;
    var progress = self.cli.interaction.progress(util.format($('Adding member %s to domain %s'), memberEmail, domainName));
    try {
      var member = dns.domains.memberships.add(domainName, memberEmail, _);
    }
    finally {
      progress.end();
    }
  ...
  deleteMember: function (context, _) {
    var self = this;
    ...
    var progress = self.cli.interaction.progress(util.format($('Removing member %s from domain %s'), memberEmail, domainName));
    try {
      dns.domains.memberships.delete(domainName, memberEmail, _);
    }
    finally {
      progress.end();
    }
